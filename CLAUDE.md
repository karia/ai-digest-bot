...
make sources-list / sources-add TITLE=.. CHANNEL_ID=.. ITEMS="url|name .." [POSTING_SCHEDULE=..] / sources-delete TITLE=..
...
4. `items` 全件について `agent.run_digest(url, since, until)` で本文テキストを生成（この時点では投稿しない）
5. 全本文から `agent.run_headline([(name, body), ...])` でヘッドライン文を生成（注目記事1〜2件に言及、リンクなし。失敗時は空文字にフォールバック）し、`slack_notifier.post_message(channel, text=headline, header=...)` で親メッセージを投稿して `ts` を取得
