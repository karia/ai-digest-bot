...
- **cost**: AWS アカウントの利用料金を Cost Explorer から取得し、当月累計・着地見込み・直近の確定日のサービス別内訳を速報値として投稿する。EventBridge（毎日 JST 23:00、`job: "cost"`）→ Lambda が起点。起動時刻に依存しない設計なので、24 時間周期でさえあれば時刻は自由。投稿は **1 メッセージ**でスレッドを作らない。Bedrock は使わない
