...
AWS の請求日は UTC 区切りで、Cost Explorer の日次バケットもそれに従います。ただし日が閉じた瞬間に金額が揃うわけではなく、反映には時間がかかります。AWS が保証するのは 1 日 1 回以上の更新で、実運用では 1 日 3 回程度、実測では締めから約 8 時間で確定していました。

そこでレポートは、**締めから `SETTLING_MARGIN`（12 時間）以上が経過した最新の UTC 日**を対象にします。起動時刻から引き算で求めるため、何時に起動しても「まだ積み上がっている最中の日」を掴みません。24 時間周期で動かしてさえいれば、起動時刻は運用の都合で自由に決められます。

対象日は起動時刻に応じて前日または前々日になります。たとえば UTC 12:00 以降に起動すれば前日、それより前なら前々日です。メッセージには対象日を日付で明示するため、どちらであっても読み違えません。

当月累計と着地見込みは、対象日ではなく起動時点の当月を追います。読み手が「当月」と言うときに指すのはそちらだからです。

### 速報値であること

反映が済んでいない値を含むため、レポートは一貫して速報値です。確定額は後から変動します。その旨をメッセージ末尾に必ず添えます。

12 時間の余裕を置いてもなお速報値である理由は、当月累計が対象日より後の未確定な日も含むこと、および AWS が請求確定まで金額を `Estimated` として扱い続けることの 2 点です。精度を上げるより、速報値だと分かる形で早く届けることを優先しています。
