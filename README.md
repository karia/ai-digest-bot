...
同じ Lambda には、AWS アカウントの利用料金を配信する **cost** ジョブと、iDeCo 等の保有商品について投資判断情報（BUY/SELL/HOLD）を配信する **advisor** ジョブも同居しています（詳細は後述の[コストレポート（cost）](#コストレポートcost)と[投資判断アドバイザー（advisor）](#投資判断アドバイザーadvisor)を参照）。
...
AWS アカウントの利用料金を Cost Explorer から取得し、当月累計・当月の着地見込み・直近の確定日のサービス別内訳を Slack へ投稿するジョブです。EventBridge（毎日 JST 23:00、`job: "cost"`）→ Lambda が起点。投稿は 1 メッセージで、スレッドは作りません。

サービス別の行には、前日比と前月同日比の増減を並べます。日額が USD 0.01 未満のサービスは末尾の 1 行にまとめますが、判定は絶対値で行うため、返金やクレジットのようなマイナス計上は畳まれずに単独行として残ります。

```
AWS コスト 2026-09-12

当月累計 $18.84  →  着地見込み $52.00
09/11 の利用 $1.33 (前日比 -$0.24 / 前月同日比 +$0.35)
...
AWS の請求日は UTC 区切りで、Cost Explorer の日次バケットもそれに従います。ただし日が閉じてから金額が反映されるまでには時間がかかります。AWS が保証するのは 1 日 1 回以上の更新で、実際の反映は 1 日 3 回程度、締めから 8 時間ほどで確定します。

レポートはこの反映待ちを織り込み、**反映が済んだと見なせる最新の UTC 日**を対象にします。起動時刻から逆算して求めるため、何時に起動しても正しい日を選びます。24 時間周期で動かしてさえいれば、起動時刻は運用の都合で決められます。

対象日は起動時刻に応じて前日または前々日になります。メッセージには対象日を日付で明示するため、どちらであっても読み違えません。当月累計と着地見込みは起動時点の当月を追います。読み手が「当月」と言うときに指すのはそちらだからです。

### 速報値であること

当月累計は対象日より後の未確定な日を含み、AWS 自身も請求が確定するまで金額を `Estimated` として扱い続けます。そのためレポートは常に速報値で、確定額は後から変動します。その旨をメッセージ末尾に必ず添えます。

### Cost Explorer の課金と精度

Cost Explorer の API は 1 リクエスト USD 0.01 課金されます。1 回の実行で `GetCostAndUsage` と `GetCostForecast` を 1 回ずつ呼ぶため、日次実行で月 USD 0.6 程度です。サービス数が増えてレスポンスがページングされる日は、全ページを読むぶんだけ加算されます。

着地見込みは `GetCostForecast` を `MONTHLY` で呼んだ結果です。月の途中を起点に指定しても API 側が期間を月全体へ正規化するため、返る値は**その月の着地額**であり、当月累計へ加算するものではありません。

Cost Explorer は未反映の日を「グループ 0 件」で返し、これは利用が実際にゼロだった日と区別できません。そのため対象日が未反映だったときは、当月累計だけを投稿し、サービス別内訳は省いて未反映である旨を添えます。
...
**3 ジョブは同一チャンネルに同居できます。** 前回投稿を読むのは digest と advisor の 2 つで、どちらも `slack_reader.find_last_post_time` でヘッダーが一致する投稿だけを対象にするため、他ジョブのスレッドを自分のものと取り違えません（cost は日付入りのヘッダーを使い、履歴も読みません）。

同居の条件は、同一チャンネル内でヘッダーが重複しないことです。これは登録時に検証されます。`make sources-add` と `make advisors-add` はどちらも、同じチャンネルに同じヘッダーを投稿する定義が既にあれば、ソースとアドバイザーのどちらであっても登録を拒否します。

なお [ADR 0001 の Consequences](docs/adr/0001-ideco-investment-advisor.md) には digest と advisor を別チャンネルに分ける旨の記述がありますが、これは digest がヘッダーで絞り込む前の制約です。
